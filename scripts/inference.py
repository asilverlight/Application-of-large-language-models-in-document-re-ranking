...
parser.add_argument("--model_name_or_path", default="/share/LMs/models--meta-llama--Llama-2-7b-chat-hf/snapshots/c1b0db933684edbfe29a06fa47eb19cc48025e93", type=str)
# parser.add_argument("--model_name_or_path", default="/share/yutao/inters/checkpoint/chat_setting_removed_query/epoch2-step2100/model", type=str)
...
def test_model(local_gpu_rank, args, tasks, label_lists, save_result=False):
    ...
    model = LlamaForCausalLM.from_pretrained(args.model_name_or_path, use_cache=True, torch_dtype=torch.bfloat16)
...
if __name__ == '__main__':
    ...
    # query description
    task = QueryDescription(name="gov2", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryDescription(name="trec_robust", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryDescription(name="trec_covid", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryDescription(name="fire", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])

    # query expansion
    task = QueryExpansion(name="gov2", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryExpansion(name="trec_robust", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryExpansion(name="trec_covid", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryExpansion(name="fire", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryExpansion(name="query2doc", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryExpansion(name="trec_cast", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryExpansion(name="trec_web", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    
    # query reformulation
    task = QueryReformulation(name="codec", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryReformulation(name="qrecc", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryReformulation(name="canard", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryReformulation(name="trec_cast", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryReformulation(name="gecor", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    
    # query clarification
    task = QueryClarification(name="mimics", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryClarification(name="mimics_duo", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    task = QueryClarification(name="clariq_fkw", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    # task = QueryClarification(name="raocq", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    
    # query subtopic generation
    task = QuerySubtopicGeneration(name="trec_web", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    
    # query suggestion
    task = QuerySuggestion(name="aol", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    
    # query matching
    task = QueryMatching(name="msrp", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append(["yes", "no"])
    
    # # query intent classification
    # task = QueryIntentClassification(name="mantis", shot=args.shots, setting="setting_overlap_new", multi_label=True)
    # tasks.append(task)
    ...
    task = QueryIntentClassification(name="orcas_i", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append(["factual", "abstain", "instrumental", "transactional", "navigational"])
    task = QueryIntentClassification(name="trec_web", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append(["faceted", "ambiguous", "navigational", "informational"])
    
    # fact verification
    # task = FactVerification(name="fever", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append(["support", "refute"])
    # task = FactVerification(name="climate_fever", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append(["support", "refute", "disputed", "not enough information"])
    task = FactVerification(name="scifact", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append(["support", "refute"])
    
    # # conversational qa
    # task = ConversationalQA(name="coqa", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    # task = ConversationalQA(name="quac", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    
    # # summarization
    # task = Summarization(name="cnndm", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    # task = Summarization(name="xsum", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    # task = Summarization(name="wikisum", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    # task = Summarization(name="multinews", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    
    # # reading comprehension
    # task = ReadingComprehension(name="squad", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    task = ReadingComprehension(name="hotpot_qa", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append([])
    # task = ReadingComprehension(name="ms_marco", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    task = ReadingComprehension(name="boolq", shot=args.shots, setting="setting_overlap_new")
    tasks.append(task)
    label_lists.append(["true", "false"])
    # task = ReadingComprehension(name="webglm_qa", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
    # task = ReadingComprehension(name="trivia_qa", shot=args.shots, setting="setting_overlap_new")
    # tasks.append(task)
    # label_lists.append([])
