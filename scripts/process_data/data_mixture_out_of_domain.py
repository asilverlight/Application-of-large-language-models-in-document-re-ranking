...
def load_overlap_data(wise_type="all", window_size=10, domain_type="task", remove_datasets=None, remove_tasks=None, task_name=None):
    all_data = {}
    if domain_type == "task":
        for cluster, names in TASK_DATASET_DICT.items():
            for name in names:
                if cluster in remove_tasks:
                    continue
    ...
    elif domain_type == "dataset":# 测试face_retrieval任务，需要fever以及climate_fever数据集作为训练集，scifact数据集作为测试集
        fact_retrieval_dataset = TASK_DATASET_DICT.get(task_name)
        cluster = task_name
        for name in fact_retrieval_dataset:
            if name in remove_datasets:
                continue
            full_name = cluster + "_" + name
            dataset_train_data = []
            for i in range(6):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--domain_type", type=str, default="task", help="whether in task-level or in database-level")
    parser.add_argument("--remove_datasets", nargs="+", default=[], help="datasets to be removed")
    parser.add_argument("--remove_tasks", nargs="+", default=[], help="tasks to be removed")
    parser.add_argument("--task_name", type=str, default="fact_retrieval", help="task name for out-of-domain data generation")# 用于数据集上的域外实验
    # 有dbpedia代表在跑基本实验，没有dbpedia代表在跑listwise长度的消融实验
    args = parser.parse_args()
    
    if not os.path.exists(args.save_path):
        if args.domain_type == "task":
            print("The type of out-of-domain experiment is task-level.")
            print("The task to be removed is: ", args.remove_tasks)
        elif args.domain_type == "dataset":
            print("The type of out-of-domain experiment is dataset-level, and the task is: ", args.task_name)
            print("The dataset to be removed is: ", args.remove_datasets)
        else:
            raise ValueError("Invalid domain_type")
        all_datasets = load_overlap_data(wise_type=args.wise_type, window_size=args.window_size, domain_type=args.domain_type, remove_datasets=args.remove_datasets, remove_tasks=args.remove_tasks, task_name=args.task_name)
